			...
			<Switch>
				...
				<Route path="/basket/:areaId/:itemId" exact>
					<Basket
						...
						order={order}
						setFaster={( place, state ) => {
							console.log(place, state, orderParams);
							const updatedParams = {...orderParams};
							if (!(place in updatedParams)) {updatedParams[place] = {}};
							updatedParams[place]['faster'] = state;
							localStorage.setItem('orderParams', JSON.stringify(updatedParams));
						...
						setTime={( place, state ) => {
							const updatedParams = {...orderParams};
							if (!(place in updatedParams)) {updatedParams[place] = {}};
							updatedParams[place]['time'] = state;
							localStorage.setItem('orderParams', JSON.stringify(updatedParams));
							setOrderParams(updatedParams);

						}}
						setSelfService={( place, state ) => {
							const updatedParams = {...orderParams};
							if (!(place in updatedParams)) {updatedParams[place] = {}};
							updatedParams[place]['selfService'] = state;
							localStorage.setItem('orderParams', JSON.stringify(updatedParams));
							setOrderParams(updatedParams);
				...
				>
					<Orders
						...
						foodAreas={FOOD_AREAS}
						setFinishedOrder={({ itemId, isCancelled }) => {
							const nextStatuses = {...orderStatuses};

							nextStatuses[itemId] = !isCancelled ? 'DONE': 'CANCELED';
